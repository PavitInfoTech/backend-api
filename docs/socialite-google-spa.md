...
3. Add an Authorized redirect URI that points to your backend callback URL, e.g.:
    ...
    - https://api.example.com/api/auth/google/callback (The `/api` prefix is required even when `API_DOMAIN=api.example.com`.)
...
Note: `API_DOMAIN` does not remove the `/api` prefix. The production callback remains `https://api.example.com/api/auth/google/callback`.
